...
import { BinanceHistoryClient } from "../binanceHistory";

test("Binance order history maps allOrders response to common Order model", async () => {
  ...
  assert.equal(requests[0].headers?.["X-MBX-APIKEY"], "key");

  assert.equal(orders.length, 1);
  assert.equal(orders[0].id, "12345");
  assert.equal(orders[0].clientOrderId, "client-1");
  assert.equal(orders[0].exchange, "binance");
...
test("Binance trade history maps myTrades response to common Fill model", async () => {
  const requests: Array<{
    method: string;
    path: string;
    query?: Record<string, string>;
  ...
  assert.equal(fills[0].exchange, "binance");
...
test("Binance history requires symbol", async () => {
  const client = new BinanceHistoryClient({
    apiKey: "key",
    apiSecret: "secret",
    httpClient: {
  ...
  await assert.rejects(
    ...
    /Binance order history requires a symbol/,
  );

  await assert.rejects(
    () => client.getTradeHistory(),
    /Binance trade history requires a symbol/,
  );
});

test("Binance order history rejects unsupported common filters", async () => {
  const client = new BinanceHistoryClient({
    apiKey: "key",
    apiSecret: "secret",
    httpClient: {
